...
token="$(node -e '
  ...
  if (request.path !== "dataFiles:list") process.exit(91);
...
if [ -z "$token" ]; then
  kind="anonymous"
elif [[ "$token" == verify-read-auth-control-* ]]; then
  kind="control"
fi

success() {
  printf '%s' '{"status":"success","value":[{"name":"SERVER-AUTHORED-PRIVATE-DATA","version":7,"updatedAt":123}],"private":"SERVER-PAYLOAD-MUST-NOT-PRINT"}' >"$output"
  printf '%s' '200'
}
...
case "${VERIFY_READ_AUTH_TEST_SCENARIO:?}" in
  enforced)
    if [ "$kind" = "known_good" ]; then success; else auth_rejection "Unauthorized: invalid read token"; fi
    ;;
  wrong_known_good)
    auth_rejection "Unauthorized: invalid read token"
    ;;
  unconfigured)
    auth_rejection "Unauthorized: CONVEX_READ_TOKEN is not configured"
    ;;
  outage)
    other_rejection
    ;;
  ...
  wrong_credential_accepted)
    if [ "$kind" = "control" ]; then success; else auth_rejection "Unauthorized: invalid read token"; fi
    ;;
  transport_outage)
...
run_case() {
  ...
  local extra_arg="${6:-}"
  local output rc
  local -a args=(--url https://stub.invalid --expect enforced)
...
run_case "verified enforcement" enforced present 0 ENFORCED --verbose
...
run_case "anonymous reads open" open present 1 OPEN
run_case "wrong credential accepted" wrong_credential_accepted present 1 OPEN

printf '%s\n' "verify-read-auth tests passed"
