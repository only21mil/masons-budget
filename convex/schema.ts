...
//
//   `transactions` / `todos` / `btcBuys` / `btcAccounts` (new): one document per
//   record. Nothing reads them yet; convex/tables.ts populates them from the
//   blobs and serves them. They exist because MC2 died on 2026-07-18 and Convex
//   is now the system of record rather than a sync target, and the blob shape
//   cannot carry that role:
//
