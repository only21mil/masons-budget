...
// ─── Test schema ─────────────────────────────────────────────────────────────
//
// The target tables belong to convex/schema.ts, which another lane is writing
// in parallel. Declaring them here keeps this suite runnable today, and the
// spread order means it stops mattering the moment the real ones land: the real
// schema's tables come LAST and win, so once schema.ts defines `transactions`
// this block becomes dead weight rather than a competing definition. If the two
// disagree, these tests go red — which is the point.

const money = v.int64(); // integer minor units. Never v.float64().

const provenance = {
  sourceFile: v.string(),
  sourceKey: v.string(),
  sourceIndex: v.float64(),
...
const migrationTables = {
  ...
  btcBuys: defineTable({
    ...
    costBasisStatus: v.union(v.string(), v.null()),
    loggedBy: v.union(v.string(), v.null()),
  }),
...
const testSchema = defineSchema({ ...migrationTables, ...baseSchema.tables });
...
describe("source keys are deterministic and total", () => {
  test("the same blob produces the same keys twice", () => {
    ...
    expect(first.docs.map((doc) => doc.sourceKey)).toEqual(second.docs.map((doc) => doc.sourceKey));
  });
  ...
  test("905 rows produce 905 distinct keys despite duplicate ids and id-less twins", () => {
    ...
    expect(new Set(docs.map((doc) => doc.sourceKey)).size).toBe(905);
  });
...
describe("owner resolution matches the domain normalizers", () => {
  ...
  test("an unrecognised owner falls back to victor, exactly as coerceOwner does", () => {
    // Deliberate parity with a rule that looks wrong in isolation: this is what
    // the clients already display, so the migrated rows must agree with it.
    expect(resolveOwner("nobody", "mason")).toBe("victor");
  });
...
describe("migrating every file", () => {
  ...
  test("child files keep their positive spend and adult files keep their negative", async () => {
    ...
    const adultSpend = rows.find((row) => row.externalId === "t-big")!;
    expect(adultSpend.amountCents).toBe(-24681357n);
  });

  test("fields no domain type mentions still survive, on the raw column", async () => {
    const t = harness();
    await seedAll(t);
    await t.mutation(api.migrateFile, { file: "bitcoin-buys", apply: true });
    const rows = await rowsIn(t, "btcBuys");
    for (const row of rows) {
      expect((row.raw as Record<string, unknown>).archimedes_request_id).toMatch(/^arch-2026-/);
    }
  });
  ...
  test("both todo dialects normalise and the wrapper is unwrapped", async () => {
    ...
    const legacy = rows.find((row) => row.externalId === "s1750000000000")!;
...
describe("running it twice", () => {
  ...
  test("a re-run does not even rewrite migratedAt", async () => {
    const t = harness();
    await seedAll(t);
    await t.mutation(api.migrateFile, { file: "bitcoin-buys", apply: true });
    const before = (await rowsIn(t, "btcBuys")).map((row) => row.migratedAt);
    await t.mutation(api.migrateFile, { file: "bitcoin-buys", apply: true });
    const after = (await rowsIn(t, "btcBuys")).map((row) => row.migratedAt);
  ...
  test("an edited blob updates the row in place instead of adding one", async () => {
    ...
    expect(rows.find((row) => row.externalId === "m003")!.amountCents).toBe(450n);
  });
});
...
describe("verification is a real check, not a formality", () => {
  ...
  test("a corrupted row fails the round-trip check even when counts and sums pass", async () => {
    ...
    await t.run(async (ctx) => {
      const rows = await ctx.db.query("transactions").collect();
      const target = rows.find((row) => row.sourceIndex === 5)!;
      // Only `raw` is touched: the count is unchanged and no money column moves,
      // so this is exactly the corruption the first two checks cannot see.
      await ctx.db.patch(target._id, {
        raw: { ...(target.raw as Record<string, unknown>), merchant: "Wrong" },
      });
    });
...
describe("contentFingerprint", () => {
  test("ignores the fields that change on every run", () => {
    const doc = { sourceKey: "id:a", amountCents: 1n, raw: { id: "a" } };
    expect(contentFingerprint({ ...doc, _id: "x", _creationTime: 1, migratedAt: 2 })).toBe(
      contentFingerprint({ ...doc, _id: "y", _creationTime: 3, migratedAt: 4 }),
    );
  });
