...
//
//   1. IDEMPOTENT.  Every projected row carries a deterministic `sourceKey`.
//      A second run finds the same keys, sees identical content, and writes
...
// ─── Reconcile with convex/tables.ts (owned by another lane) ────────────────
//
 ...
 */
export function resolveOwner(raw: unknown, fallbackOwner: string): string {
  if (raw === undefined) return fallbackOwner;
 ...
 *
 * `raw` is carried verbatim on every row, deliberately. The typed columns are a
 ...
 *
 * `migratedAt` is excluded on purpose: including it would make every row differ
 * from itself on every run, which would turn the idempotent second pass into a
 * full rewrite and make "unchanged: 905" impossible to assert.
 */
const VOLATILE_FIELDS = new Set(["_id", "_creationTime", "migratedAt"]);
 ...
 *   round trip  — reconstructs the source array from the stored `raw` fields in
 *                 `sourceIndex` order and canonical-JSON compares it to the
...
export function verifyProjection(
  ...
  for (let index = 0; index < compared; index += 1) {
    if (canonicalJson(ordered[index]!.raw) !== canonicalJson(blobRows[index]!)) {
...
/** Every already-migrated row for one source file, keyed by sourceKey. */
async function readMigrated(
  ...
  for (const doc of existing) {
    if (doc.sourceFile !== file) continue;
    byKey.set(String(doc.sourceKey), doc);
  }
  return byKey;
}
...
export const status = internalQuery({
  ...
  handler: async (ctx) => {
    ...
    for (const source of MIGRATION_SOURCES) {
      ...
      const migrated = await readMigrated(ctx, source.table, source.file);
      files.push({
        file: source.file,
        table: source.table,
...
export const migrateFile = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingByKey = await readMigrated(ctx, source.table, source.file);
    ...
    for (let index = cursor; index < end; index += 1) {
      ...
      const existing = existingByKey.get(String(doc.sourceKey));

      if (existing === undefined) {
        inserted += 1;
        if (apply) await ctx.db.insert(source.table, { ...doc, migratedAt: now });
        continue;
      }
      ...
      updated += 1;
      if (apply) await ctx.db.patch(existing._id, { ...doc, migratedAt: now });
    }
    ...
    if (apply && wholeFileInOneBatch) {
      ...
      const storedByKey = await readMigrated(ctx, source.table, source.file);
