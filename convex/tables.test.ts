// convex/tables.ts — row tables, migration, visibility and auth.
//
// The four things these tests exist to stop, in order of how much they would
// cost if they happened:
//
//   1. The migration mutating `dataFiles`. Every shipped client still reads the
//      blobs and this is the only copy of the family's financial record.
...
const fn = {
  ...
  >,
  migrateFromBlob: "tables:migrateFromBlob" as unknown as FunctionReference<
    "mutation",
    "public",
    { sourceFile: string; offset?: number; limit?: number; token?: string },
    {
      sourceFile: string;
      kind: string;
      total: number;
...
async function migrateAll(t: T) {
  for (const name of [
    ...
    let offset = 0;
    for (;;) {
      const result = await t.mutation(fn.migrateFromBlob, {
        sourceFile: name,
        offset,
      });
...
describe("the blob path is untouched", () => {
  it("a full migration does not write dataFiles, syncVersions or todoTombstones", async () => {
    await t.run(async (ctx) => {
      await ctx.db.insert("todoTombstones", { id: "gone-1", deletedAt: 1 });
    });
    ...
    // Byte-for-byte, including every `version`. A bumped version would make
    // every shipped client re-download the whole file for no reason.
...
describe("owner is first class, and the two visibility rules keep their widths", () => {
  ...
  it("a garbage owner string in a CHILD file falls back to the child, not to an adult", async () => {
    ...
    await t.mutation(fn.migrateFromBlob, { sourceFile: "maddox-transactions" });
  ...
  it("a recognised owner inside a file still wins over the file's default", async () => {
    await replaceDataFile(t, "todos", [
      { id: "todo-3", title: "Mason chore", owner: "mason", updated_at: "2026-07-12T09:00:00Z" },
    ]);
    await t.mutation(fn.migrateFromBlob, { sourceFile: "todos" });
...
describe("migration", () => {
  it("is idempotent — a second run updates in place and adds nothing", async () => {
    await migrateAll(t);
    const first = await t.query(fn.rowCounts, {});
    ...
    const rerun = await t.mutation(fn.migrateFromBlob, {
      sourceFile: "transactions",
    });
    expect(rerun.inserted).toBe(0);
    expect(rerun.updated).toBe(3);

    await migrateAll(t);
    expect(await t.query(fn.rowCounts, {})).toEqual(first);
  });

  it("pages, and reports total / nextOffset / done honestly", async () => {
    const page1 = await t.mutation(fn.migrateFromBlob, {
      sourceFile: "transactions",
      limit: 2,
    });
    ...
    const page2 = await t.mutation(fn.migrateFromBlob, {
      sourceFile: "transactions",
  ...
  it("refuses an unknown source file rather than guessing an owner", async () => {
    await seedDataFile(t, "surprise-transactions", []);
    await expect(
      t.mutation(fn.migrateFromBlob, { sourceFile: "surprise-transactions" }),
    ).rejects.toThrow(/Unknown source file/);
  });

  it("reports a missing blob as done with nothing migrated", async () => {
    const fresh = testTables();
    openGates();
    const result = await fresh.mutation(fn.migrateFromBlob, {
      sourceFile: "transactions",
    });
    expect(result).toMatchObject({ total: 0, inserted: 0, done: true });
  });

  it("counts a record with no id as skipped instead of dropping it silently", async () => {
    await seedDataFile(t, "maddox-transactions", [
      { date: "2026-07-08", merchant: "Sweets", amount: 5, category: "Fun" },
    ]);
    const result = await t.mutation(fn.migrateFromBlob, {
  ...
  it("does not resurrect a todo the blob path already tombstoned", async () => {
    await t.run(async (ctx) => {
      await ctx.db.insert("todoTombstones", { id: "todo-1", deletedAt: 1 });
    });
    const result = await t.mutation(fn.migrateFromBlob, { sourceFile: "todos" });
    expect(result.skipped).toBe(1);

    const todos = await t.query(fn.listTodos, { viewer: "victor" });
    expect(todos.map((todo) => todo.todoId)).toEqual(["todo-2"]);
  });

  it("accepts todos stored as { todos: [...] } as well as a bare array", async () => {
    // Both shapes have been written to this file over its life.
    await replaceDataFile(t, "todos", {
      todos: [{ id: "wrapped-1", title: "Wrapped", updated_at: "2026-07-13T09:00:00Z" }],
    });
    const result = await t.mutation(fn.migrateFromBlob, { sourceFile: "todos" });
    expect(result).toMatchObject({ total: 1, inserted: 1 });
    expect(
      (await t.query(fn.listTodos, { viewer: "victor" })).map((todo) => todo.todoId),
    ).toEqual(["wrapped-1"]);
  });

  it("carries todo done/flag through the canonical normalizer", async () => {
    await migrateAll(t);
    const open = await t.query(fn.listTodos, { viewer: "victor", done: false });
    const closed = await t.query(fn.listTodos, { viewer: "victor", done: true });
    expect(open.map((todo) => todo.todoId)).toEqual(["todo-1"]);
    expect(closed.map((todo) => todo.todoId)).toEqual(["todo-2"]);
  });
});
...
describe("auth: the gates in tables.ts match the gates in dataFiles.ts", () => {
  ...
  const writeEntryPoints = [
    ...
    {
      name: "migrateFromBlob",
      call: (token?: string) =>
        t.mutation(fn.migrateFromBlob, { sourceFile: "transactions", token }),
    },
  ] as const;
  ...
  describe("the hatch outranks the token, exactly as it does for dataFiles", () => {
    ...
    it("sync: a set token is IGNORED while ALLOW_TOKENLESS_SYNC=true", async () => {
      ...
      await expect(
        t.mutation(fn.migrateFromBlob, { sourceFile: "transactions" }),
      ).resolves.toBeDefined();
    });
