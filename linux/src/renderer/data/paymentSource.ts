...
export function paymentSourceBlockReason(selection: PaymentSourceSelection): string | null {
  ...
  if (classification === "fiat_card") {
    return isSpend
      ? null
      : `${paymentSourceLabel(source)} can only be used on a spending transaction outside Income.`
  }
  if (!isSpend && !isIncome) {
...
export function transactionSubmission(
  ...
  const retiredBitcoinPosting = isRetiredBitcoinSource(legacy) &&
