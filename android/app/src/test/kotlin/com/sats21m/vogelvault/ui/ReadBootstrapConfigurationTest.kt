...
class ReadBootstrapConfigurationTest {
    ...
    fun `busy state disables replay and reset is presence only`() {
        ...
        assertFalse(application.todoCredentialPresent)
    ...
    @Test
    fun `already cleared enrollment reset succeeds as a no-op`() {
        val enrollment = ReadOnlyBootstrapEnrollment(application)

        assertEquals(BootstrapAccess.NONE, enrollment.reset())
        assertEquals(BootstrapAccess.NONE, enrollment.currentAccess())
        assertEquals(1, application.removeCalls)
    }
...
class RecordingReadBootstrapApplication : VaultApplication() {
    ...
    var connectCalls = 0
    var removeCalls = 0
    var todoCredentialPresent = false
