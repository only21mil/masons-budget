#!/usr/bin/python3
"""Operator-only prepare/install/remove for the reviewed MBP build boundary.
...
def prepare(checkout, commit, bundle):
    ...
    manifest = {'schema': 1, 'workflow_sha': commit, 'files': hashes}
...
def load_bundle(bundle, expected):
    ...
    require(set(manifest) == {'schema', 'workflow_sha', 'files'} and manifest['schema'] == 1
...
def install(bundle, expected):
    ...
    for parent in (INSTALL.parent, STATE.parent, SUDOERS.parent):
        if not parent.exists() and parent in (INSTALL.parent, SUDOERS.parent):
            ...
            parent.chmod(0o755)
        protected(parent)
    for path in (INSTALL, STATE, SUDOERS):
    ...
    require(account.pw_uid == BUILDER_UID and account.pw_gid == BUILDER_GID and account.pw_shell == '/usr/bin/false', 'account verification failed')
...
def remove(expected_commit):
    ...
    require(set(os.listdir(STATE)) <= {'installation.json', 'sudoers.candidate', 'supervisor.lock'}, 'remaining build roots require reviewed recovery')
    account = pwd.getpwnam('buzzbuild')
    require(account.pw_uid == BUILDER_UID and account.pw_gid == BUILDER_GID, 'account identity changed')
    if INSTALL.exists():
        protected(INSTALL)
