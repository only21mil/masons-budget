...
def run(argv: list[str], *, output: Path | None = None, input_data: bytes | None = None,
        extra: dict | None = None, confidential: bool = False) -> bytes:
...
def sign(args: argparse.Namespace) -> None:
    ...
    def notarize(path: Path, kind: str) -> dict:
        submission = output/f"{args.arch}-notary-{kind}-submission.json"
        log = output/f"{args.arch}-notary-{kind}-log.json"
        ...
        submission.write_bytes(result.stdout)
        ...
        require(result.returncode == 0 and data.get("status") == "Accepted" and report.get("status") == "Accepted", "notarization not Accepted; retained diagnostic JSON")
    ...
    app_notary = notarize(app_zip, "app")
    verification = {}
    ...
    write_json(output/f"mac-release-{args.arch}.json", {
        "schema": "buzz-macos-release-v1", "source": {"repository": "only21mil/buzz", "sha": args.source},
        "version": args.version, "arch": args.arch, "target": f"{args.arch}-apple-darwin",
...
def main() -> int:
    ...
    parser.add_argument("command", choices=("inputs", "pack", "prepare", "sign", "cleanup"))
