...
function fixture({ internal = false, publicLink = false, other = false, member = true, assigned = true,
  state = 'INVITED', beta = 'IN_BETA_TESTING' } = {}) {
  const writes = [];
  const api = async (path, method = 'GET', body) => {
    ...
    if (p === '/v1/betaGroups') return { data: [{ id: GROUP, type: 'betaGroups', attributes: { name: 'Buzz private beta', isInternalGroup: internal, publicLinkEnabled: publicLink, hasAccessToAllBuilds: false } }] };
    if (p.endsWith('/relationships/betaTesters')) return { data: [...(member ? [{ id: TESTER }] : []), ...(other ? [{ id: 'other' }] : [])] };
    if (p.endsWith('/relationships/builds')) return { data: assigned ? [{ id: BUILD }] : [] };
