...
class BoundaryTests(unittest.TestCase):
    ...
    def test_hash_and_record_without_python311_file_digest(self):
        # Exercise empty, short, exact-block and multi-block files on Python 3.9+.
        with patch.object(hashlib, 'file_digest', create=True,
                          side_effect=AssertionError('Python 3.11 helper must not be used')):
            for size in (0, 3, 1024 * 1024, 1024 * 1024 + 17):
                with self.subTest(size=size):
                    data = (b'abc123\x00' * (size // 7 + 1))[:size]
                    path = self.root / 'payload.bin'
                    path.write_bytes(data)
                    expected = hashlib.sha256(data).hexdigest()
                    self.assertEqual(release.sha(path), expected)
                    self.assertEqual(release.record(path),
                                     {'name': 'payload.bin', 'sha256': expected, 'size': size})
    ...
    def test_workflow_secret_boundary(self):
        ...
        self.assertIn('runs-on: [self-hosted, macOS, ARM64, macbook-pro-m5]', build)
        self.assertIn('buzz_macos_build_boundary.py', build)
        self.assertNotIn('repository: only21mil/buzz', build)
        self.assertNotIn('activate-hermit@', build)
